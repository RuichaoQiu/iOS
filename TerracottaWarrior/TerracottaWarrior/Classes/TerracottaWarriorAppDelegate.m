//
//  ZYG006AppDelegate.m
//
//  Copyright 2011 qrc. All rights reserved.
//

#import "TerracottaWarriorAppDelegate.h"
#import "cocos2d.h"
#import "StartScene.h"

@implementation TerracottaWarriorAppDelegate

@synthesize window;

- (void) applicationDidFinishLaunching:(UIApplication*)application
{
	// Init the window
	//NSLog(@"xixi");	
	window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
	
	
	// cocos2d will inherit these values
	[window setUserInteractionEnabled:YES];	
	[window setMultipleTouchEnabled:YES];
	
	mpViewController = [[MyMPViewController alloc] init];
	[window addSubview:mpViewController.view];
	[window makeKeyAndVisible];	
	
	
	
	
	
	//将原先代码放回试验后还是不行，sprite还是黑色背景
	//	if( ! [CCDirector setDirectorType:CCDirectorTypeDisplayLink] )
	//		[CCDirector setDirectorType:CCDirectorTypeDefault];
	//	
	//	// Use RGBA_8888 buffers
	//	// Default is: RGB_565 buffers
	//	[[CCDirector sharedDirector] setPixelFormat:kPixelFormatRGBA8888];
	//	
	//	// Create a depth buffer of 16 bits
	//	// Enable it if you are going to use 3D transitions or 3d objects
	//	//	[[CCDirector sharedDirector] setDepthBufferFormat:kDepthBuffer16];
	//	
	//	// Default texture format for PNG/BMP/TIFF/JPEG/GIF images
	//	// It can be RGBA8888, RGBA4444, RGB5_A1, RGB565
	//	// You can change anytime.
	//	[CCTexture2D setDefaultAlphaPixelFormat:kTexture2DPixelFormat_RGBA8888];
	//	
	//	// before creating any layer, set the landscape mode
	//	[[CCDirector sharedDirector] setDeviceOrientation:CCDeviceOrientationLandscapeLeft];
	//	[[CCDirector sharedDirector] setAnimationInterval:1.0/60];
	//	[[CCDirector sharedDirector] setDisplayFPS:YES];
	//	
	//	// create an openGL view inside a window
	//	[[CCDirector sharedDirector] attachInView:window];	
	//	[window makeKeyAndVisible];		
	//	NSLog(@"xixi");	
	//	
	//	[[CCDirector sharedDirector] runWithScene: [StartScene ShowScene]];
	
	
	
	
	
	//	[[CCDirector sharedDirector] setPixelFormat:kPixelFormatRGBA8888];
	//	[CCTexture2D setDefaultAlphaPixelFormat:kTexture2DPixelFormat_RGBA8888];
	//	[[CCDirector sharedDirector] setDeviceOrientation:CCDeviceOrientationLandscapeLeft];
	//	[[CCDirector sharedDirector] setAnimationInterval:1.0/60];
	//	[[CCDirector sharedDirector] setDisplayFPS:YES];
	//	//UIView *anyView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
	//	//[[CCDirector sharedDirector] attachInView:anyView];
	//	//[[[UIApplication sharedApplication] keyWindow] addSubview:anyView];
	//	//[[CCDirector sharedDirector] runWithScene: [StartScene ShowScene]];
}

+ (void) ContinueLaun
{
	/*
	 if( ! [CCDirector setDirectorType:CCDirectorTypeDisplayLink] )
	 [CCDirector setDirectorType:CCDirectorTypeDefault];
	 */
	// Use RGBA_8888 buffers
	// Default is: RGB_565 buffers
	[[CCDirector sharedDirector] setPixelFormat:kPixelFormatRGBA8888];
	
	// Create a depth buffer of 16 bits
	// Enable it if you are going to use 3D transitions or 3d objects
	//	[[CCDirector sharedDirector] setDepthBufferFormat:kDepthBuffer16];
	
	// Default texture format for PNG/BMP/TIFF/JPEG/GIF images
	// It can be RGBA8888, RGBA4444, RGB5_A1, RGB565
	// You can change anytime.
	[CCTexture2D setDefaultAlphaPixelFormat:kTexture2DPixelFormat_RGBA8888];
	
	// before creating any layer, set the landscape mode
	[[CCDirector sharedDirector] setDeviceOrientation:CCDeviceOrientationLandscapeLeft];
	[[CCDirector sharedDirector] setAnimationInterval:1.0/120];
	//[[CCDirector sharedDirector] setDisplayFPS:YES];
	
	// create an openGL view inside a window
	UIView *anyView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 480)];
	[anyView setMultipleTouchEnabled:YES];
	[[CCDirector sharedDirector] attachInView:anyView];
	[[[UIApplication sharedApplication] keyWindow] addSubview:anyView];
	[[CCDirector sharedDirector] runWithScene: [StartScene ShowScene]];
}


- (void)applicationWillResignActive:(UIApplication *)application {
	[[CCDirector sharedDirector] pause];
}

- (void)applicationDidBecomeActive:(UIApplication *)application {
	[[CCDirector sharedDirector] resume];
}

- (void)applicationDidReceiveMemoryWarning:(UIApplication *)application {
	[[CCTextureCache sharedTextureCache] removeUnusedTextures];
}

- (void)applicationWillTerminate:(UIApplication *)application {
	[[CCDirector sharedDirector] end];
}

- (void)applicationSignificantTimeChange:(UIApplication *)application {
	[[CCDirector sharedDirector] setNextDeltaTimeZero:YES];
}

- (void)dealloc {
	[[CCDirector sharedDirector] release];
	[window release];
	[super dealloc];
}

@end
